    ...
    return (
        <div id="detail">
            <div id="movie-container">
                {movie ? (
                    <>
                        <img src={`${process.env.REACT_APP_API_URL}${movie.poster_url.replace(/\\/g, "/")}`} alt={movie.title} className="banner" />
                        <h1>{movie.title}</h1>
                            ...
                            <div className="info-right">
                                ...
                                <p>
                                    ...
                                    {Array.isArray(movie.nations) && movie.nations.length > 0 ? (
                                        movie.nations
                                            .map((nation) => (
                                                <span key={nation.nation_id} className="clickable" onClick={() => handleNavigate("nation", nation.nation_id)}>
                                                    {nation.name}
                                                </span>
                                            ))
                                            .reduce((prev, curr) => [prev, ", ", curr])
                                    ) : movie.nation ? (
                                        <span className="clickable" onClick={() => handleNavigate("nation", movie.nation.nation_id)}>
                                            {movie.nation.name}
                                        </span>
                                    ) : (
                                        "Chưa cập nhật"
                                ...
                                <p>
                                    ...
                                    {movie.directors && movie.directors.length > 0
                                        ? movie.directors
                                              .map((director) => (
                                                  <span key={director.director_id} className="clickable" onClick={() => handleNavigate("directors", director.director_id)}>
                                                      {director.name}
                                                  </span>
                                              ))
                                              .reduce((prev, curr) => [prev, ", ", curr])
                                ...
                                <p>
                                    ...
                                    {movie.actors && movie.actors.length > 0
                                        ? movie.actors
                                              .map((actor) => (
                                                  <span key={actor.actor_id} className="clickable" onClick={() => handleNavigate("actors", actor.actor_id)}>
                                                      {actor.name}
                                                  </span>
                                              ))
                                              .reduce((prev, curr) => [prev, ", ", curr])
                        ...
                        <fieldset id="tap">
                            ...
                            {movie.episodes.map((item) => (
                                <button key={item.episode_id} onClick={() => handleTapClick(item.episode_id)}>
