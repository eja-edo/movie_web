...
const movieAPI = {
    ...
    getFilmData: async (id, navigate, retry = false) => {
        try {
            ...
            const response = await fetch(`${process.env.REACT_APP_API_URL}/api/movies/detailMovie/`, {
    ...
    getVideoData: async (id1, id2, navigate, retry = false) => {
        try {
            const accessToken = localStorage.getItem('accessToken');
            const response = await fetch(`${process.env.REACT_APP_API_URL}/api/movies/film/`, { // Use template literal
