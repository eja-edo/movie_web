...
const CreateDisplayList = memo((films) => {
  console.log("Props films nhận được:", films);
  const componentRef = useRef(null);
  const [dimensions, setDimensions] = useState({ width: 0, height: 0 });

  const filmItems = Object.values(films)[0];
  //   console.log("Danh sách phim:", filmItems);
  //   console.log("em bill", filmItems);
  ...
  const back = (button) => {
    ...
    let trans =
      parseInt(
        getComputedStyle(div).getPropertyValue("--trans").replace("px", ""),
        10
      ) || 0; // Đảm bảo trans là số
  ...
  const xem_ngay = async () => {
    const firstEpisode = await movieAPI.getFirstEpisode(infoFilm.id, navigate);

    if (firstEpisode) {
      navigate(
        `/film/${infoFilm.id}/${firstEpisode.current_episode.episode_id}`
      );
    } else {
  ...
  const molen = (element) => {
    ...
    console.log("bill2", index);
    const trans =
      parseInt(
        getComputedStyle(divParent)
    ...
    <div className="display" ref={componentRef}>
      ...
      <div className="video" ref={divVideoRef}>
        <video
          src={
            infoFilm.video != ""
              ? process.env.REACT_APP_API_URL + infoFilm.video
              : ""
          }
          muted
          loop
          ...
          poster={
            infoFilm.poster != ""
              ? process.env.REACT_APP_API_URL + infoFilm.poster
              : ""
          }
          ref={videoRef}
        />
          ...
          {filmItems.map((item, index) => (
            <div key={index} data-key={index} className="film-container">
              <a
                ...
                onClick={(event) => handleClickImg(event.currentTarget)}
              >
                <img
