  ...
  return (
    <ul className={`dropdown-menu multi-column-dropdown ${layoutClass}`}>
      {items.map((item, index) => {
        const itemId =
          item.genre_id || item.nation_id || item.id || `unknown-${index}`;
        ...
        return (
          <li key={`${type}-${itemId}`}>
            <Link
              to={`/category/${type}?${queryString}`}
              onClick={() => {
                // Đảm bảo onClose được gọi khi người dùng chọn một mục
                if (onClose) onClose();

                // Đóng dropdown trên mobile
                if (window.innerWidth <= 768) {
                  const mobileMenu = document.querySelector("#min_menu ul");
                  if (mobileMenu) {
